<template>
  <v-layout row wrap class="executivereports" v-bind="genBreadcrumbs">
    <v-flex xs12>
      <div class="_date">
        <span>{{ today }}</span>
      </div>
                    ...
                    <v-flex xs12> 
                      ...
                      <v-list subheader>
                        <v-list-tile avatar :to="('executivereports/' + item._id)" v-for="item in itemRealtimeOrgList" :key="item._id">
                          ...
                          <v-list-tile-action>
                            <span style="font-weight: bold;">{{ formatPercentOrga(item.summary) }}</span>
                          </v-list-tile-action>
                        </v-list-tile>
                      </v-list>
                    </v-flex>
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        ...
        organization: 'infoorganizations/current'
      }),
      today () {
    ...
    created () {
      this.$store.commit('presencestodaysummary/clearAll')
      this.$store.commit('presencestoday/clearAll')
      this.$store.commit('presencestodayorganizations/clearAll')
      ...
      if (this.$route.params.id) {
        this.$store.dispatch('infoorganizations/get', this.idOrganization)
      }
      this.initSetDefault()
      this.fingerToday()
